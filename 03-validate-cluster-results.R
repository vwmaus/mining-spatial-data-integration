# -----------------------------------------------------------------
# SCRIPT: 03-validate-cluster-results.R
# -----------------------------------------------------------------
#
# This script performs an external validation of the final clustered results.
# It compares the commodity assignments generated by script 02 against an
# independent, expert-curated reference dataset (Werner et al. 2020).
#
# Workflow:
# 1.  Reads the final `mine_polygons.gpkg` generated by the optimization script.
# 2.  Downloads, pre-processes, and reads the Werner et al. (2020) validation data.
# 3.  Performs a spatial join to match clusters from this workflow to the
#     reference polygons.
# 4.  For each match, it calculates commodity list similarity metrics
#     (Jaccard, Dice, Overlap Coefficient).
# 5.  It saves the raw comparison data (`clusters_check_werner.csv`).
# 6.  It generates a LaTeX summary table (`tbl-similarity-metrics.tex`)
#     of the validation statistics for the manuscript.
# 7.  It exports a GeoPackage (`mismatching_cluster_visual_check.gpkg`) of
#     clusters with low match scores for visual inspection in GIS.
